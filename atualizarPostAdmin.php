    ...
    if (isAdmin($_SESSION['tipo'])){ //SE O USUÁRIO LOGADO FOR DO TIPO ADMINISTRADOR
      ...
      if ((!isset($_GET['idSecao'])) || (!isset($_GET['idPost']))) { //se não tiver nenhum id passado pela url deve-se redirecionar o usuário para a página inicial
        ...
        if (isset($_POST["atualizar"])) {
          ...
          $SQL = "UPDATE post SET titulo = '" . $titulo . "', conteudo = '" . $conteudo . "', img = '" . $img . "', cadastro = 1, autorAlteracao = " . $autorAlteracao . ", dataAlteracao = CURRENT_TIME(), autorAprovacao = " . $autorAprovacao . ", dataAprovacao = CURRENT_TIME() WHERE idPost = ".$idPost;

          if ($con->query($SQL) === TRUE) {

            $sql = "UPDATE postpendente SET titulo = '" . $titulo . "', conteudo = '" . $conteudo . "', img = '" . $img . "', aprovacao = 1, cadastro = 1, autorAlteracao = " . $autorAlteracao . ", dataAlteracao = CURRENT_TIME() WHERE idPostPendente = ".$idPost;
