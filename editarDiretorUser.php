  ...
  if (isset($_SESSION["login"])) { //SE EXISTIR AUTENTICAÇÃO
    if (($_SESSION['tipo'] != 'socio') && ($_SESSION['tipo'] != 'dependente') && (isAdmin($_SESSION['tipo']) == false)) {
      ...
      if (!isset($_GET['id'])) { //se não tiver nenhum id passado pela url deve-se redirecionar o usuário para a página inicial
        ...
        $sql = "SELECT aprovacao FROM diretoriapendente WHERE aprovacao = 0 AND idDiretorPendente = " . $id;
        ...
        $sql = "SELECT * FROM diretoria WHERE idDiretor = $id";
        ...
        if (isset($_POST["atualizar"])) {
            ...
            $sql = 'UPDATE diretoriapendente SET nome = "' . $nome . '", email = "' . $email . '", descricao = "' . $descricao . '", cargo = "' . $cargo . '", img = "' . $img . '", autor = ' . $autor . ', dataAlteracao = CURRENT_TIME(), aprovacao = 0 WHERE idDiretorPendente = ' . $id . ';';
                    
            if ($con->query($sql) === TRUE) {
              echo "<script>alert('Atualização realizada com sucesso! Por favor, aguarde a aprovação da diretoria.');</script>";
          ...
          <div class="row">
            <div class="col-md-12"> 
              ...
              <div class = "post">
                ...
                <hr>
                <form class="form-horizontal" method="POST" action="editarDiretorUser.php?id=<?php echo $id; ?>" enctype="multipart/form-data">
