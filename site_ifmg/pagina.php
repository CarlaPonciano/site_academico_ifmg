<?php
  ...
  $sqlPagina = "SELECT pagina.nome, conteudo, data_alteracao, usuario.nome as usuario, usuario.nome as autor_aprovacao, data_aprovacao FROM pagina 
  JOIN usuario ON id_autor = usuario.id WHERE pagina.id = " . $id;
  ...
  if ($resultPagina->num_rows > 0) { // Exibindo cada linha retornada com a consulta
    while ($exibirPagina = $resultPagina->fetch_assoc()){
      ...
      $sqlAutorAprovacao = "SELECT pagina.nome FROM pagina JOIN usuario ON id_autor_aprovacao = usuario.id WHERE pagina.id= " . $id; 
