<?php
  ...
  $SQL = "SELECT SecaoPosts_id, titulo, conteudo, img, nome as usuario, dataAlteracao, dataAprovacao, dataPostagem FROM post 
          JOIN usuario ON autorAlteracao = usuario.id WHERE idPost = $id;";
  $result = $conn->query($SQL);

  if ($result->num_rows > 0) { // Exibindo cada linha retornada com a consulta
    while ($exibir = $result->fetch_assoc()){
      $idSecao = $exibir["SecaoPosts_id"];
      ...
      $sql = "SELECT nome FROM post JOIN usuario ON autorAprovacao = usuario.id WHERE idPost = $id;";

      $result = $conn->query($sql);

      if ($result->num_rows > 0) { // Exibindo cada linha retornada com a consulta
        while ($exibir = $result->fetch_assoc()){
          $autorAprovacao = ucwords($exibir["nome"]);
        }
      }
    }
  }else{
        ...
        <div class="col-lg-8">
          ...
          if (isset($_SESSION['email'])){
            if ($_SESSION['tipo'] == 2){
              ...
              $sql = "SELECT aprovacao FROM postpendente WHERE idPostPendente = $id";
          ...
          <?php
          /*    $sql = "SELECT * FROM comentario WHERE idPost = $id ORDER BY dataEnvio DESC";
