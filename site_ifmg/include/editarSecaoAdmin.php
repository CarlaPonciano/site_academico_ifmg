  ...
  if (isset($_SESSION["login"])) { //SE EXISTIR AUTENTICAÇÃO
    if (($_SESSION['tipo'])==2){ //SE O USUÁRIO LOGADO FOR DO TIPO ADMINISTRADOR
      ...
      if (!isset($_GET['id'])) { //se não tiver nenhum id passado pela url deve-se redirecionar o usuário para a página inicial
        ...
        $sqlSecao = "SELECT * FROM secao WHERE idSecao = $id";
        ...
        if (isset($_POST["atualizar"])) {
          ...
          $sqlAtualizarSecao = "UPDATE secao SET nome = '".$nomeAtualizado."', conteudo = '".$conteudoAtualizado."', dataAlteracao = CURRENT_TIME(), autor = '".$autor."', dataAprovacao = CURRENT_TIME(), autorAprovacao = '".$autor."', exibir = " . $exibirAtualizado . " WHERE idSecao = ".$id;
          
          if ($conn->query($sqlAtualizarSecao) === TRUE) {
            echo "<script>alert('Atualização realizada com sucesso!');</script>";
            echo "<script>window.location = 'secao.php?id=" . $id . "';</script>";
          } else {
            echo "Erro: " . $sqlAtualizarSecao . "<br>" . $conn->error;
          }
          $con->close();
