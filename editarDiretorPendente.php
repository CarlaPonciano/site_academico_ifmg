  ...
  if (isset($_SESSION["login"])) { //SE EXISTIR AUTENTICAÇÃO
    if (isAdmin($_SESSION['tipo'])){ //SE O USUÁRIO LOGADO FOR DO TIPO ADMINISTRADOR
      ...
      if (!isset($_GET['id'])) { //se não tiver nenhum id passado pela url deve-se redirecionar o usuário para a página inicial
        ...
        $sql = "SELECT dp.idDiretorPendente, dp.nome, dp.email, dp.descricao, dp.cargo, dp.img, p.usuario, dp.autor, dp.dataAlteracao FROM diretoriapendente AS dp JOIN perfil AS p ON autor = idPerfil WHERE idDiretorPendente = " . $id;

        $result = $con->query($sql);

        if ($result->num_rows > 0) { // Exibindo cada linha retornada com a consulta
          while ($exibir = $result->fetch_assoc()){
            $id = $exibir["idDiretorPendente"];
            ...
            $autor = ucwords($exibir["usuario"]);
            $autorId = $exibir["autor"];
        ...
        if (isset($_POST["atualizar"])) {
            ...
            $sql = 'UPDATE diretoria SET nome = "' . $nome . '", email = "' . $email . '", descricao = "' . $descricao . '", cargo = "' . $cargo . '", img = "' . $img . '", autor = ' . $autorId . ', dataAlteracao = "' . $datetimeAlt . '", autorAprovacao = ' . $autorAprovacao . ', dataAprovacao = CURRENT_TIME() WHERE idDiretor = ' . $id . ';';
                    
            if ($con->query($sql) === TRUE) {

              $sql = "UPDATE diretoriapendente SET aprovacao = 1 WHERE idDiretorPendente = ".$_GET["id"];
          ...
          <div class="row">
            <div class="col-md-12"> 
              ...
              <div class = "post">
                ...
                <hr>
                <form class="form-horizontal" method="POST" action="editarDiretorPendente.php?id=<?php echo $id; ?>" enctype="multipart/form-data">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-12">
                      <input type="text" class="form-control" id="nome" name="nome" value="<?php echo $nome; ?>" required></input>
                    </div>
                  </div> 
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-12">
                      <input type="text" class="form-control" id="cargo" name="cargo" value="<?php echo $cargo; ?>" required></input>
                    </div>
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    <img style="width: 350px;" src="upload/img-diretor/<?php echo $img; ?>" alt=""></label>
                    <div class="col-sm-12">
