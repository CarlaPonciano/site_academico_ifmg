  ...
  if (isset($_SESSION["login"])) { //SE EXISTIR AUTENTICAÇÃO
    if (isAdmin($_SESSION['tipo'])){ //SE O USUÁRIO LOGADO FOR DO TIPO ADMINISTRADOR
      ...
      if (!isset($_GET['id'])) { //se não tiver nenhum id passado pela url deve-se redirecionar o usuário para a página inicial
        ...
        $sqlPagina = "SELECT * FROM paginapendente JOIN perfil ON autor = idPerfil WHERE idPaginaPendente =" . $id;
        ...
        if (isset($_POST["atualizar"])) {
          ...
          $sqlAtualizarPagina = "UPDATE pagina SET nome = '".$nomeAtualizado."', conteudo = '".$conteudoAtualizado."', dataAlteracao = '". $datetimeAlt ."', autor = '".$autorId."', dataAprovacao = CURRENT_TIME(), autorAprovacao = '".$autorAprovacao."' WHERE idPagina = ".$id;
