    ...
    if (($_SESSION['tipo'] != 'socio') && ($_SESSION['tipo'] != 'dependente') && (isAdmin($_SESSION['tipo']) == false)) {
      ...
      if ((!isset($_GET['idSecao'])) || (!isset($_GET['idPost']))) { //se não tiver nenhum id passado pela url deve-se redirecionar o usuário para a página inicial
        ...
        if (isset($_POST["atualizar"])) {
          ...
          $sql = "UPDATE postpendente SET titulo = '" . $titulo . "', conteudo = '" . $conteudo . "', img = '" . $img . "', aprovacao = 0, cadastro = 1, autorAlteracao = " . $autorAlteracao . ", dataAlteracao = CURRENT_TIME() WHERE idPostPendente = ".$idPost;
