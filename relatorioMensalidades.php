...
//recebe os valores enviados pelo formulário
    ...
    $referenciaInicial = $_POST['referenciaInicialModal'];

    $referenciaFinal = $_POST['referenciaFinalModal'];

    $dataReferenciaInicial = DateTime::createFromFormat('Y-m-d', $referenciaInicial . "-01");

    $dataReferenciaFinal = DateTime::createFromFormat('Y-m-d', $referenciaFinal . "-01");
    ...
    $sql = "SELECT nome, cpf, matriculaAAA, idMensalidade, valor, dataPagamento, dataReferenciaInicial, dataReferenciaFinal 
              ...
              WHERE dataPagamento BETWEEN '" . $dataReferenciaInicial->format('Y-m-d') . "' 
              AND '" . $dataReferenciaFinal->format('Y-m-d') . "';";
    ...
    $totalMensalidades = number_format($totalMensalidades,2);//formata o número com duas casas decimais

    $html .= '</table>

              <div class="bor"></div><h4>VALOR TOTAL RECEBIDO: R$ ' . str_replace(".", ",", $totalMensalidades) . '</h4>';
