  ...
  if (isset($_SESSION["login"])) { //SE EXISTIR AUTENTICAÇÃO
    if (isAdmin($_SESSION['tipo'])){ //SE O USUÁRIO LOGADO FOR DO TIPO ADMINISTRADOR
      ...
      if (!isset($_GET['id'])) { //se não tiver nenhum id passado pela url deve-se redirecionar o usuário para a página inicial
        ...
        $sql = "SELECT * FROM secaopendente JOIN perfil ON autor = idPerfil WHERE idSecaoPendente = $id";

        $result = $con->query($sql);

        if ($result->num_rows > 0) { // Exibindo cada linha retornada com a consulta
          while ($exibir = $result->fetch_assoc()){
            $id = $exibir["idSecaoPendente"];
            $nome = ucwords($exibir["nome"]);
            $conteudo = $exibir["conteudo"];
            $autor = $exibir["usuario"];
        ...
        if (isset($_POST["atualizar"])) {
          ...
          $sql = "UPDATE secao SET nome = '".$nome."', conteudo = '".$conteudo."', dataAlteracao = '". $datetimeAlt ."', autor = '".$autorId."', dataAprovacao = CURRENT_TIME(), autorAprovacao = '".$autorAprovacao."' WHERE idSecao = ".$_GET["id"];
          
          if ($con->query($sql) === TRUE) {

            $sql = "UPDATE secaopendente SET aprovacao = 1 WHERE idSecaoPendente = ".$_GET["id"];
          ...
          <div class="row">
            <div class="col-md-12"> 
              ...
              <div class = "post">
                ...
                <hr>
                <form class="form-horizontal" method="POST" action="editarSecaoPendente.php?id=<?php echo $id; ?>" enctype="multipart/form-data">
