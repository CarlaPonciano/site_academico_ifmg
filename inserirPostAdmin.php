      ...
      if (!isset($_GET['id'])) { //se não tiver nenhum id passado pela url deve-se redirecionar o usuário para a página inicial
        ...
        if (isset($_POST["cadastrar"])) {
          ...
          if ($numFile<=0) {
            ...
            $SQL = 'INSERT INTO post (idSecaoPosts, titulo, conteudo, img, autorAlteracao, dataAlteracao, autorAprovacao, dataAprovacao, cadastro, dataPostagem) VALUES (' . $idSecao . ', "' . $titulo . '","' . $conteudo . '","' . $img . '",' . $autor . ', CURRENT_TIME(), ' . $autor . ', CURRENT_TIME(), 1, CURRENT_TIME());';

            if ($con->query($SQL) === TRUE){
              //verifica se o comando foi executado com sucesso

              $sql = 'INSERT INTO postpendente (idSecaoPosts, titulo, conteudo, img, autorAlteracao, dataAlteracao, aprovacao, cadastro) VALUES (' . $idSecao . ', "' . $titulo . '","' . $conteudo . '","' . $img . '",' . $autor . ', CURRENT_TIME(), 1, 1);';
