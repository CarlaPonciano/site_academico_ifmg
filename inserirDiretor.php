    ...
    if (isAdmin($_SESSION['tipo'])){ //SE O USUÁRIO LOGADO FOR DO TIPO ADMINISTRADOR
      ...
      if (isset($_POST["cadastrar"])) {
        ...
        if ($numFile<=0) {
          ...
          $SQL = 'INSERT INTO diretoria (nome, email, descricao, cargo, img, autor, dataAlteracao, autorAprovacao, dataAprovacao) VALUES ("' . $nome . '","' . $email . '","' . $descricao . '","' . $cargo . '", "' . $img . '", ' . $autor . ', CURRENT_TIME(), ' . $autor . ', CURRENT_TIME());';

          if ($con->query($SQL) === TRUE){
            //verifica se o comando foi executado com sucesso

            $SQL = 'INSERT INTO diretoriapendente (nome, email, descricao, cargo, img, autor, dataAlteracao, aprovacao) VALUES ("' . $nome . '","' . $email . '","' . $descricao . '","' . $cargo . '", "' . $img . '", ' . $autor . ', CURRENT_TIME(), 1);';
            ...
            if ($con->query($SQL) === TRUE){
              ...
              echo "<script>alert('Erro ao cadastrar diretor!');</script>";
