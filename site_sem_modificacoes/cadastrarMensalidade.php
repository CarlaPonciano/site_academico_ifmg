...
<?php
  $valorMensal = 0;
  $valorAnual = 0;
  $sql = "SELECT valor, descricao FROM valorMensalidade;";

  $resulta = $con->query($sql);
  if ($resulta->num_rows > 0) {
    // output data of each row
    while ($rows = $resulta->fetch_assoc()) {
      if ($rows['descricao'] === 'mensal') {
          $valorMensal = $rows['valor'];
      }

      if ($rows['descricao'] === 'anual') {
          $valorAnual = $rows['valor'];
      }
    }
  }
?>
...
<form enctype="multipart/form-data" role="form" data-toggle="validator" action="inserirMensalidade.php" method="post">
        ...
        <div class="card-body">
          ...
          <div class="form-row">
            ...
            <div class="form-group col-md-6">
              ...
              <input required pattern="^[0-9]{1,4}.[0-9]{2}$" type="text" class="form-control" id="valor" readonly="true" value="<?php echo number_format($valorMensal, 2); ?>" name="valor">
            </div>
    ...
    <button type="submit" name="insertMensalidade" class="btn btn-primary">Cadastrar</button>
