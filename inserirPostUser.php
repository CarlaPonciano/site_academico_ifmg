      ...
      if (!isset($_GET['id'])) { //se não tiver nenhum id passado pela url deve-se redirecionar o usuário para a página inicial
        ...
        if (isset($_POST["cadastrar"])) {
          ...
          if ($numFile<=0) {
            ...
            $sqlPostPendenteSemImagem = 'INSERT INTO postpendente (idSecaoPosts, titulo, conteudo, img, autorAlteracao, dataAlteracao, aprovacao, cadastro) VALUES (' . $idSecao . ', "' . $titulo . '","' . $conteudo . '","' . $img . '",' . $autor . ', CURRENT_TIME(), 0, 0);';

            if ($con->query($sqlPostPendenteSemImagem) === TRUE){

              $sqlPostSemImagem = 'INSERT INTO post (idSecaoPosts, titulo, conteudo, img, autorAlteracao, dataAlteracao, cadastro, dataPostagem) VALUES (' . $idSecao . ', "' . $titulo . '","' . $conteudo . '","' . $img . '",' . $autor . ', CURRENT_TIME(), 0, CURRENT_TIME());';
            ...
            for ($i=0; $i < $numFile; $i++) {
              ...
              if ($error != 0) { //se não houver erro ao carregar a imagem
                ...
                if (move_uploaded_file($tmp, $folder."/".$img)) {
                  ...
                  $sqlPostPendente = 'INSERT INTO postpendente (idSecaoPosts, titulo, conteudo, img, autorAlteracao, dataAlteracao, aprovacao, cadastro) VALUES (' . $idSecao . ', "' . $titulo . '","' . $conteudo . '","' . $img . '",' . $autor . ', CURRENT_TIME(), 0, 0);';

                  if ($con->query($sqlPostPendente) === TRUE){

                    $sqlPost = 'INSERT INTO post (idSecaoPosts, titulo, conteudo, img, autorAlteracao, dataAlteracao, cadastro, dataPostagem) VALUES (' . $idSecao . ', "' . $titulo . '","' . $conteudo . '","' . $img . '",' . $autor . ', CURRENT_TIME(), 0, CURRENT_TIME());';
